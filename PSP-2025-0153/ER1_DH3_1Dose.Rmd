...
#length(unique(idata.allID$ID)) #451
...
- df.ER.IDQ only include the last observation
```{r}
# df.exp <- readRDS("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/df_exp.rds")
...
setwd( "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24") 
 ...
 ids <- df.exp %>%
  ...
  selected_ids <- sample(ids, 10)
    ...
    exp.10 <- df.exp %>% 
     ...
     ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/Typical plots for exposure matrix.tiff",
        plot = COMBINE_TP, 
...
ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM plot_no strata.tiff",
        plotp, 
...
plot1 <- KM_plot_T(KM.Ctrough1DIDQ, df.ER.IDQ,"Ctrough1D_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_Ctrough1D.tiff")

plot2 <- KM_plot_T(KM.Cavg1DIDQ, df.ER.IDQ,"Cavg1D_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_Cavg1D.tiff")

plot3 <- KM_plot_T(KM.CavgOEDIDQ, df.ER.IDQ,"CavgOED_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_CavgOED.tiff")
plot4 <- KM_plot_T(KM.Cavg1WOEDIDQ, df.ER.IDQ,"Cavg1WOED_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_Cavg1W.tiff")

plot5 <- KM_plot_T(KM.Cavg2WOEDIDQ, df.ER.IDQ,"Cavg2WOED_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_Cavg2W.tiff")

plot6 <- KM_plot_T(KM.CavgTEIDQ, df.ER.IDQ,"CavgTE_IDQ",600,
       "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM_False_CavgTE.tiff")

combine_3 <- grid.arrange(plot1$plot, plot2$plot, plot3$plot, plot4$plot,plot5$plot, plot6$plot, ncol = 3)

ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/KM Plot 3 groups.tiff",
      plot = combine_3, 
...
ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/Logistic regression plot_conc.tiff",
      plot = combine_2, 
...
#6 Dosing history 
##6.1 event vs nonevent
```{r}
EvsN_plot <- function(data_e, data_n, y_variable, y_label, plot_title) {
  ggplot() +
    geom_line(data = data_e, aes_string(x = "DAY", y = y_variable, color = "Type"), size=1) +
    geom_line(data = data_n, aes_string(x = "DAY", y = y_variable, color = "Type"), size=1) +
    labs(y = y_label, x = "Days since first dose", color = "Type") +
    ggtitle(plot_title) +
    scale_color_manual(values = c("Event" = "red", "Non-Event" = "blue")) +
    custom_theme +
...
## Split the df.ER into two dataset(GID.e and GID.n)

df.e <- df.ER %>% 
         filter(EVENT == 1) #last dose

##Abtract unique ID for the patients treated with cabo and has an event
df.ID.e <- df.e %>%
           select(ID) %>%
           distinct()

##get the Patients treated with cabo and without an event
df.ID.all <- df.ER %>%
             select(ID) %>%
             distinct()

df.ID.n <- anti_join(df.ID.all, df.ID.e, by = "ID")

#length(df.ID.all$ID) "1000" 
#length(df.ID.e$ID) "338" 
#length(df.ID.n$ID) "662" 

GID.e <- data.frame()
for (i in 1:length(df.ID.e$ID)){
  b <- subset(df.ER,df.ER$ID==df.ID.e$ID[i])
  GID.e <- rbind(GID.e,b)
}

GID.n <- data.frame()
for (i in 1:length(df.ID.n$ID)){
  b <- subset(df.ER,df.ER$ID==df.ID.n$ID[i])
  GID.n <- rbind(GID.n,b)
}

GID.e <- GID.e %>%
         select(-SUBJID,-SEQ,-EVENT,-LAST)

GID.n <- GID.n %>%
         select(-SUBJID,-SEQ,-EVENT,-LAST) 

df.mean.e <- aggregate(GID.e,by=list(GID.e$DAY),FUN = mean)
df.mean.n <- aggregate(GID.n,by=list(GID.n$DAY),FUN = mean)
df.mean.e$Type <- "Event"
df.mean.n$Type <- "Non-Event"

Dhistory.daily <- EvsN_plot(df.mean.e, df.mean.n, "DOSE", "Average Dose (mg)", "Average daily dose history")

ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/Average daily dose history comparison between E vs nonE.tiff",
      plot = Dhistory.daily, 
      width = 6, height = 5, dpi = 400, units = "in")
```
##6.2 Dosing history for df.ER
- Distribution of dose regimen for all time points
- Only correct for 4 groups!!!!
```{r}
...
ggsave("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/Dose vs event summary plot.tiff",
             plot = DE.plot, 
...
df9$`1`[is.na(df9$`1`)]<-"0"
write.csv(df9,file = "C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/dose frequency table.csv") 

```


#7 Replicate simulation by 1000 times_EO1

##7.1 Loop function for ER analysis
...
```{r}
univ_con_IDQ <- readRDS("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/univ_con_IDQ.rds")
univ_cat_IDQ <- readRDS("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/univ_cat_IDQ.rds")
wilcox_test <- readRDS("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/wilcox_test.rds")
logistic_test <- readRDS("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis/ER1_DH3_1Dose/ER1_DH3_HL7_EO1_ii24/logistic_test.rds")
...
#8 For EO2 having breaks issue_did not get CoxPh cat
##8.1 loop function
```{r}
loop <- function(DI, Case.fix, seed, parameter.451, dose.451, event.shape, event.scale){
...
##8.2 Using random seed to perform the ER analysis
### input the initial dose and parameter dataset
```{r}
setwd("C:/Users/Xuefen.Yin/OneDrive - FDA/ER1 analysis")
# mrgsolve model
...
```
##8.4  abstract and summary slope values for logistic regression
```{r}
slope_results <- extract_slope(logistic_test, Slope = "Slope", variables = con_var)
summary_results <- summarize_slopes(slope_results)
...
saveRDS(logit_results, "logit results.rds")
```
## 8.5  abstract and summary beta values 
```{r}
Beta.cont <- extract_beta_cont(univ_con_IDQ, con_var)
coxPH_odd <- Beta.cont %>% mutate(hazard.ratio = exp(Beta))
...
Beta.cont_df <- Beta.cont %>%
  ...
  summarise(
    ...
    Cox.Mean = mean(Beta),
    Cox.Median = median(Beta),
    Cox.q0.025 = quantile(Beta, 0.05), 
    Cox.q0.975 = quantile(Beta, 0.95)  
    )%>%
   mutate(Beta_variable = "Beta")
